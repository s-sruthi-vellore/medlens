        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-xl bg-slate-800 border border-slate-700 shadow-2xl z-50 py-2 text-xs">
                  ...
                  {SAMPLE_PATIENTS.map((p, idx) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 hover:bg-slate-700/70 flex flex-col gap-0.5 transition-colors ${
                        patient.id === p.id ? 'bg-sky-500/10 border-l-2 border-sky-400 text-sky-300' : 'text-slate-300'
                      }`}
                    >
                      ...
                      <div className="text-[11px] text-slate-400 truncate">
                        {p.age}y {p.sex} • {p.existingConditions.join(', ') || 'No chronic history'}
                      </div>
                    </button>
                  ))}
                  <div className="border-t border-slate-700/60 mt-1 pt-1 px-2">
                    <button
                      ...
                      className="w-full text-left px-2 py-1.5 rounded bg-sky-500/20 text-sky-300 hover:bg-sky-500/30 flex items-center gap-1.5 text-[11px] font-semibold"
                    >
